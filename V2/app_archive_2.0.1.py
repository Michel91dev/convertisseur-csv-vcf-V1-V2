...
def convertir_csv_en_vcf(contenu_csv: str, mappings_colonnes: Dict[str, str], note_commune: Optional[str] = None, delimiter: str = ';') -> str:
    ...
    try:
        ...
        if not prenom_nom_colonne or prenom_nom_colonne not in colonnes_disponibles:
            st.error("La colonne nom/prénom n'est pas correctement configurée dans le mapping.")
            return ""
...
def afficher_documentation_champs() -> None:
    ...
    with st.expander("Documentation des champs"):
        ...
        Le nom complet de la personne, avec prénom et nom de famille. L'application sépare automatiquement 
    ...
    **Voici comment utiliser cette application :**
    ...
    if uploaded_file is not None:
        ...
        try:
            ...
            if colonnes:
                ...
                with col1:
                    ...
                    if 'nom' in mappings_valides:  # Vérifier que le champ obligatoire est présent
                ...
                with col2:
                    ...
                    if 'nom' in mappings_valides and st.button("Appliquer configuration par défaut"):
                ...
                with st.form("form_mappings"):
                    ...
                    # Ajouter une option vide pour les champs optionnels
                    colonnes_avec_vide = [''] + colonnes
                    ...
                    # Bouton pour enregistrer les mappings
                    submit_button = st.form_submit_button("Enregistrer la configuration")

                    if submit_button:
                        # Vérifier qu'au moins le champ nom est configuré
                        if 'nom' not in nouveaux_mappings or not nouveaux_mappings['nom']:
                            st.error("Vous devez au minimum configurer le champ 'Nom et prénom'.")
                        else:
        ...
        if st.button("Convertir en VCF"):
            ...
            if not st.session_state.mappings_colonnes or 'nom' not in st.session_state.mappings_colonnes:
                ...
                if vcf_content:
                    ...
                    st.success("Conversion réussie ! Vous pouvez télécharger le fichier VCF.")
                    st.info("""
                    **Pour importer dans Contacts sur macOS :**
                    ...
                    4. Choisissez le fichier VCF téléchargé
                    """)
                else:
